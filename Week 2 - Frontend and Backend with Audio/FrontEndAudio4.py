...
class SpanishQuizLovableApp:
    ...
    def start_new_quiz(self, retries=3):
        """Start a new quiz by fetching questions from the backend"""
        for attempt in range(retries):
            try:
                print(f"Attempt {attempt + 1} to connect to backend...")
                ...
                if response.status_code == 200:
                    ...
                    if 'questions' in data:
                        ...
                        error_msg = f"Unexpected response format. Expected 'questions' key but got: {list(data.keys())}"
                        logging.error(error_msg)
                        if attempt == retries - 1:  # Only show error on last attempt
                            self.show_error("Backend Error", error_msg)
                else:
                    error_msg = f"Failed to fetch questions. Status: {response.status_code}"
                    logging.error(error_msg)
                    if attempt == retries - 1:  # Only show error on last attempt
                        self.show_error("Backend Error", error_msg)
                ...
                error_msg = f"Failed to parse backend response: {str(e)}"
                logging.error(error_msg)
                if attempt == retries - 1:  # Only show error on last attempt
                    self.show_error("Data Error", error_msg)
            except Exception as e:
